...
func InstallAddonInWorld(addonPath string, worldDir string) ([]*Pack, error) {
	...
	for _, pack := range packs {
		...
		if err != nil {
			return installed, fmt.Errorf("failed to install pack %q: %w", pack.Name(), err)
		}
		pack.Path = packTargetDir
		...
		if err != nil {
			return installed, fmt.Errorf("failed to register pack %q: %w", pack.Manifest.Header.Name, err)
		}
