...
func TestInstallAddon_Bundle(t *testing.T) {
	...
	installedPacks, err := InstallAddon(getAddonFixturePath(t, "foobar.mcaddon"), tempServerDir)
	if err != nil {
...
func TestInstallAddon_StandaloneMcpack(t *testing.T) {
	...
	installedPacks, err := InstallAddon(getAddonFixturePath(t, "solo.mcpack"), tempServerDir)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestInstallAddon_Errors(t *testing.T) {
	...
	t.Run("invalid addon file", func(t *testing.T) {
		_, err := InstallAddon(getAddonFixturePath(t, "corrupt.mcaddon"), tempServerDir)
		if err == nil {
	...
	t.Run("addon with no manifests", func(t *testing.T) {
		_, err := InstallAddon(getAddonFixturePath(t, "no_manifest.zip"), tempServerDir)
		if err == nil {
	...
	t.Run("invalid server directory", func(t *testing.T) {
		...
		_, err := InstallAddon(getAddonFixturePath(t, "foobar.mcaddon"), newInvalidServerDir)
		if err == nil {
	...
	t.Run("server with level-name set but worlds dir not yet created should fail", func(t *testing.T) {
		...
		_ /*installedPacks*/, err := InstallAddon(getAddonFixturePath(t, "foobar.mcaddon"), tempServerDir)
		if err == nil {
